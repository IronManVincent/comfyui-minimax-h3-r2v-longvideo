...
| 本工作流 v1.4.1（上一稳定版） | ~36.6 分钟 | 8 步 | 音频质量最佳，耗时较长 |
| **本工作流 v1.4.2（当前主推）** | **~20-24 分钟** | **4 步** | **性能提升40%，镜头语言更丰富，音频可接受** |

> 💡 **采样步数选择**：
> - **4步（默认，推荐）**：速度快，画面镜头丰富，音频有轻微衰减但可接受
> - **8步**：音频质量最佳，耗时约36分钟，适合对音质要求高的场景
>
> Larry Turbo LoRA 支持 4-8 步采样，步数越多音频质量越好，但耗时线性增加。
...
- **🚀 Larry Turbo 加速**：4 步采样（默认），相比官方 25 步配置提速约 2-3 倍
...
- **🎬 青春活力镜头语言**：头→胸→臀→腿→上半身ending的身体部位特写序列，避免中远景人脸崩坏
...
### 输出视频（5段×3秒 = 15秒，0.3MP，9:16竖屏，v1.4.2 4步采样）
...
<p align="center">
  ...
  <br>
  <i>👆 点击图片播放演示视频（15秒，纯乐器配乐，5段分镜头，4步采样）</i>
</p>
...
- 总时长：15秒
- 采样步数：4步（simple调度器）
- 耗时：约20-24分钟（RTX 5060 8GB）
- 音频：纯乐器配乐（无人声）

**分镜头设计**（青春活力版，极特写避免人脸崩坏）：
1. 头部脸部极特写，灵动表情，眨眼微笑，发丝飞扬
2. 肩颈胸部极特写，姿态生动，轮廓光勾勒
3. 腰臀曲线极特写，侧身动态，自然摆动
4. 腿部极特写，自然交叉动作，环绕镜头
5. 上半身ending pose，活力收尾，镜头微推

---

## 📋 版本更新

### v1.4.2（当前稳定版，2026-09-02）

核心改进：
- **采样步数 8→4 步**：性能提升约40%，耗时从36分钟降至20-24分钟
- **镜头语言全面优化**：青春活力版提示词，头→胸→臀→腿→上半身ending的身体部位特写序列
- **极特写构图**：禁止中远景，避免0.3MP低分辨率下人脸崩坏细节丢失
- **人物动态增强**：姿态生动有张力，发丝飞扬，表情灵动，不再僵硬慢动作
- **音频提示词优化**：明确纯乐器配乐约束，减少伪语言人声生成
- **随机种子randomize**：每次生成不同结果，避免重复

测试效能（RTX 5060 8GB）：
- 5段×3秒=15秒视频，0.3MP分辨率，4步采样，总耗时约 20-24 分钟

### v1.4.1（上一稳定版，2026-09-01）
...
需要 **ComfyUI v0.30.0 及以上**（含原生 H3 AV-mask 支持和官方 MiniMax-H3 节点）。

推荐使用秋叶整合包，或从官方仓库更新到最新版。

> ⚠️ **版本兼容性说明**：ComfyUI-H3-Motion-Context v0.4.0 要求 ComfyUI v0.34.0+；如使用 v0.33.2 等旧版本，请将 Motion-Context 降级到 v0.3.1（兼容新旧两种 H3 布局）。
...
1. 打开 ComfyUI 网页界面
2. 将 `workflows/minimax_h3_r2v_longvideo_v1.4.2.json` 拖入画布
...
3. **采样步数**：BasicScheduler 节点，默认 4 步（性能优先），对音质要求高可改为 8 步
4. **随机种子**：每个分镜头有独立 seed，默认 randomize

### 第六步：生成

1. 点击 **Queue Prompt**
2. 等待生成（5段×3秒约20-24分钟，RTX 5060 8GB，4步采样）
...
| 采样步数 | **4** | 默认4步（性能优先）；对音质要求高可改为8步；Larry Turbo推荐范围4-8步 |
...
在通用提示词中明确声明："纯乐器配乐，无人声，无唱歌，无对话，无旁白"。工作流默认已包含此约束。如仍出现人声，可尝试增加采样步数到8步。

</details>

<details>
<summary><b>Q: 4步采样音频有衰减，能升回8步吗？</b></summary>

可以。将 BasicScheduler 节点的步数从 4 改为 8 即可。8步音频质量最佳，但耗时增加到约36分钟。适合对音质要求高的正式产出。
...
4. **音频链式参考固有局限**：第2-5段音频参考第1段尾帧，存在轻微节奏重复和音质衰减，属MiniMax-H3模型特性
...
- [x] **性能与镜头优化（v1.4.2）**：4步采样提速40%，青春活力版镜头语言，极特写避免人脸崩坏
