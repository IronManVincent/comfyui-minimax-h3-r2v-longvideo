...
基于 ComfyUI 的 MiniMax-H3 参考生视频（Reference-to-Video）**长视频工作流项目**。提供两种实现方式：**导演台自定义节点**（动态分镜头增删，v2.0.0）和**多节点工作流**（固定5段标准人像写真，v2.1.0），集成 Larry Turbo 加速，在 RTX 5060 8GB 入门级显卡上可稳定生成带音频的长视频。

> 🆕 **v2.1.0 更新**：多节点工作流版本，修复音频连续性问题（context_audio上下文传递），性能优化27%，镜头语言全面升级（景别层次化+时间分段+多样化运镜）。
>
> 📌 **v2.0.0 导演台版本**：自定义节点包，支持动态增删分镜头，git clone 即装即用。
...
| 方案 | 视频时长 | 耗时 | 采样步数 | 说明 |
|------|---------|------|----------|------|
| 社区导演台（默认配置） | 6秒 | ~50-60 分钟 | 25 步 | 无加速，官方默认配置 |
| 本项目 v1.4.2（静态工作流） | 15秒 | ~20-24 分钟 | 4 步 | 固定5段分镜头，性能优化版 |
| 本项目 v2.0.0（导演台） | 6秒 | ~13 分钟 | 5 步 | 动态分镜头增删，bypass模式 |
| **本项目 v2.1.0（多节点工作流）** | **15秒** | **~41 分钟** | **4 步** | **音频连续性修复，context_audio上下文传递，镜头语言优化** |
...
- **🔗 音频无缝衔接**：v2.1.0多节点工作流采用context_audio上下文传递，各段振幅稳定无衰减；v2.0.0导演台采用trim首尾静音 + 100ms等功率交叉淡入淡出
...
### v2.1.0（当前版本，2026-09-25）

**多节点工作流版本**：在v1.4.2基础上持续优化，非导演台版本

核心改进：
- **音频连续性修复（重大）**：sg_b子图新增context_audio输入端口，主图建立4条段间audio连接，实现视频+音频双上下文传递，各段振幅稳定无衰减
- **性能优化**：context_length从22帧降到5帧，audio_context_length从24降到12，两参数提升为子图输入端口可灵活调整，耗时优化27%
- **镜头语言优化**：景别层次化（大特写→中近景→侧面特写→中景→中近景），时间分段设计（0-1s/1-2s/2-3s），运镜多样化
- **4步采样**：保持Larry LoRA最低推荐步数

测试效能（RTX 5060 8GB）：
- 5段×3秒=15秒视频，0.3MP分辨率，4步采样，总耗时约 41 分钟

### v2.0.0（导演台版本，2026-09-03）
